...
for c=1:j
    r=randi(100); %choose randomly the 4 targets (set to 1) in the 100 cells
    while x(r)==1
        r=randi(100);
    end
    x(r)=1;
...
%Aware attacks
for c=1:h %choose randomly the 2 attacks (set to a=1/2*y) in the 25 sensors
    r=randi(q);
    while a(r)==1/2*y(r)
        r=randi(q);
    end
...
for k=1:T
    ...
    x=A*x; %changing in time
    y=D*x+eta+a; %changing in time
